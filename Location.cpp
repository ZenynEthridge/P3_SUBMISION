...
Location::Location(std::string line, int year) // double argument for govt text
{
    const unsigned int
...
std::string Location::outputEntry(int i)  // returns a string for out file purposes
{
...
void Location::printMoreData() { // like print data but also prints rainfall entries
	printData();
...
// |!| NEEDS UNIT TEST 
bool Location::hasEntry(RainDataEntry entry) { // checks if entry for given month is present
...
// |!| NEEDS UNIT TEST
void Location::replaceEntry(RainDataEntry entry) // replaces entry if that month and year exist
{
